...
static int kswapd(void *p)
{
	...
	if (!cpumask_empty(cpumask)) {
		// zte add
		if (nr_cpu_ids == 8) {
			struct cpumask newcpumask;

			cpumask_clear(&newcpumask);
			cpumask_set_cpu(4, &newcpumask);
			cpumask_set_cpu(5, &newcpumask);
			cpumask_set_cpu(6, &newcpumask);
			set_cpus_allowed_ptr(tsk, &newcpumask);
		} else {
	...
	for_each_node_state(nid, N_MEMORY) {
		...
		if (cpumask_any_and(cpu_online_mask, mask) < nr_cpu_ids) {
			...
			if (nr_cpu_ids == 8) {
				...
				cpumask_set_cpu(5, &newcpumask);
				cpumask_set_cpu(6, &newcpumask);
				set_cpus_allowed_ptr(pgdat->kswapd, &newcpumask);
			} else {
