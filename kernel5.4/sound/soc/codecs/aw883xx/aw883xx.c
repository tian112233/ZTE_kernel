...
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
struct zlog_client *zlog_aw883xx_client = NULL;
static struct zlog_mod_info zlog_aw883xx_dev = {
...
static void zte_log_aw883xx_w(char *log_buf)
{
	if (!zlog_aw883xx_client) {
		pr_err("%s zlog register client zlog_aw883xx_dev fail2\n", __func__);
		return;
	}
	pr_info("enter %s!\n", __func__);
	zlog_client_record(zlog_aw883xx_client, "smartpa_reason = %s\n", log_buf);
	zlog_client_notify(zlog_aw883xx_client, ZLOG_SMARTPA_I2C_W_ERROR_NO);
}

static void zte_log_aw883xx_r(char *log_buf)
{
	if (!zlog_aw883xx_client) {
		pr_err("%s zlog register client zlog_aw883xx_dev fail2\n", __func__);
		return;
	...
	pr_info("enter %s!\n", __func__);
	zlog_client_record(zlog_aw883xx_client, "smartpa_reason = %s\n", log_buf);
	zlog_client_notify(zlog_aw883xx_client, ZLOG_SMARTPA_I2C_R_ERROR_NO);
}
#endif
...
i2c_master_error:
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	snprintf(report, sizeof(report), "Write:i2c 0x%x\n", aw883xx->i2c->addr);
	zte_log_aw883xx_w(report);
	aw_dev_err(aw883xx->dev, "%s: dsm report, %s\n", __func__, report);
#endif
	kfree(data);
	data = NULL;
	return ret;
...
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	snprintf(report, sizeof(report), "read:i2c 0x%x\n", aw883xx->i2c->addr);
	zte_log_aw883xx_r(report);
	aw_dev_err(aw883xx->dev, "%s: dsm report, %s\n", __func__, report);
#endif
		return ret;
...
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	zlog_aw883xx_client = zlog_register_client(&zlog_aw883xx_dev);
	if (!zlog_aw883xx_client) {
		aw_dev_err(&i2c->dev, "zlog register client zlog_aw883xx_dev fail\n");
	}
#endif
