					...
					uint16_t gain_value)
{
	...
	monitor_vol = aw881xx_reg_val_to_db(gain_value);
...
void aw881xx_monitor_start(struct aw881xx_monitor *monitor)
{
	...
	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_SYSCTRL, &reg_val);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: failed to read cur mode, ret=%d\n",
				__func__, ret);
		return;
	}

	if (reg_val & AW881XX_BIT_SYSCTRL_RCV_MODE) {
		aw_dev_dbg(aw881xx->dev, "%s: receiver mode no need to start monitor\n",
				__func__);
		return;
	}
...
void aw881xx_monitor_stop(struct aw881xx_monitor *monitor)
{
	...
	aw881xx->volume_desc.monitor_volume = 0;
...
int aw881xx_monitor_parse_fw(struct aw881xx_monitor *monitor,
				uint8_t *data, uint32_t data_len)
{
	int ret = -EINVAL;
...
static int aw_monitor_real_time_update_cfg(struct aw881xx *aw881xx)
{
	...
	ret = aw881xx_monitor_parse_fw(&aw881xx->monitor, aw_monitor_cnt->data,
				aw_monitor_cnt->len);
	if (ret < 0)
