...
#define AW881XX_DRIVER_VERSION "v1.9.0"
...
static int aw881xx_append_i2c_suffix(char *format, const char **change_name,
...
static int aw881xx_append_channel_suffix(char *format, const char **change_name,
			struct aw881xx *aw881xx)
{
	char buf[64] = { 0 };
	uint8_t channel = aw881xx->channel;

	snprintf(buf, 64, format, *change_name, channel);
	*change_name = aw881xx_devm_kstrdup(aw881xx->dev, buf);
	if (!(*change_name))
		return -ENOMEM;
...
static int aw881xx_set_hw_volume(struct aw881xx *aw881xx, uint32_t value)
{
	uint16_t reg_value = 0;
	uint16_t volume = 0;
	uint32_t real_value = 0;

	volume = AW_GET_MIN_VALUE(value, AW_FADE_OUT_TARGET_VOL);
	real_value = aw881xx_db_val_to_reg(volume);
	/*cal real value*/
...
static int aw881xx_get_hw_volume(struct aw881xx *aw881xx, uint32_t *value)
{
	uint16_t reg_value = 0;
	uint32_t real_value = 0;
...
int aw881xx_dev_set_volume(struct aw881xx *aw881xx, uint32_t value)
{
	int ret = -EINVAL;
	uint32_t hw_vol = 0;
	struct aw_volume_desc *vol_desc = &aw881xx->volume_desc;

	hw_vol = value + vol_desc->init_volume;

	ret = aw881xx_set_hw_volume(aw881xx, hw_vol);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "set volume failed");
		return ret;
	}
...
int aw881xx_dev_get_volume(struct aw881xx *aw881xx, uint32_t *get_vol)
{
	int ret = -EINVAL;
	uint32_t hw_vol = 0;
	struct aw_volume_desc *vol_desc = &aw881xx->volume_desc;

	ret = aw881xx_get_hw_volume(aw881xx, &hw_vol);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "read volume failed");
		return ret;
	}
...
{
	int i = 0;
	struct aw_volume_desc *desc = &aw881xx->volume_desc;
	int32_t fade_in_vol = desc->ctl_volume;

	if (!aw881xx->fade_en)
		return;

	if (g_fade_step == 0 || g_fade_in_time == 0) {
		aw881xx_dev_set_volume(aw881xx, fade_in_vol);
		return;
	}

	/*volume up*/
	for (i = AW_FADE_OUT_TARGET_VOL; i >= fade_in_vol; i -= g_fade_step) {
		aw881xx_dev_set_volume(aw881xx, i);
		usleep_range(g_fade_in_time, g_fade_in_time + 10);
	}

	if (i != desc->ctl_volume)
		aw881xx_dev_set_volume(aw881xx, desc->ctl_volume);
...
static void aw881xx_fade_out(struct aw881xx *aw881xx)
{
	...
	if (g_fade_step == 0 || g_fade_out_time == 0) {
		aw881xx_dev_set_volume(aw881xx, AW_FADE_OUT_TARGET_VOL);
		return;
	}

	i = start_volume;
	for (i = desc->ctl_volume; i <= AW_FADE_OUT_TARGET_VOL; i += g_fade_step) {
		aw881xx_dev_set_volume(aw881xx, i);
...
{
	if (aw881xx->cali_desc.cali_re != AW_ERRO_CALI_VALUE) {
		aw881xx_set_cali_re_to_dsp(&aw881xx->cali_desc);
	} else {
		aw_dev_info(aw881xx->dev, "%s: no set re=%d\n",
			__func__, aw881xx->cali_desc.cali_re);
	}
}
				...
				uint8_t *data, uint32_t len)
{
	...
	for (i = 0; i < len; i += 4) {
		...
		if (reg_addr == AW881XX_REG_HAGCCFG6) {
			read_vol = reg_val >> AW881XX_VOL_REG_SHIFT;
			aw881xx->volume_desc.init_volume =
				aw881xx_reg_val_to_db(read_vol);
		}
	...
	if (aw881xx->cur_prof != aw881xx->set_prof) {
		...
		aw881xx->volume_desc.ctl_volume = 0;
	} else {
		/*keep control volume when PA start with sync mode*/
		aw881xx_dev_set_volume(aw881xx, aw881xx->volume_desc.ctl_volume);
	}
...
static int aw881xx_device_start(struct aw881xx *aw881xx)
{
	...
	/*close mute*/
	if (aw881xx_cali_check_result(&aw881xx->cali_desc))
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	aw_dev_info(aw881xx->dev, "ucontrol->value.integer.value[0]=%d",
		vol_desc->ctl_volume);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	aw881xx_dev_set_volume(aw881xx, compared_vol);
...
static void aw881xx_request_firmware_file(struct aw881xx *aw881xx)
{
	...
	ret =  request_firmware(&cont, AW881XX_ACF_FILE, aw881xx->dev);
	if (ret) {
...
static void aw881xx_load_fw(struct aw881xx *aw881xx)
{
#ifdef AW_SYNC_LOAD
		/*sync loading*/
		aw881xx_request_firmware_file(aw881xx);
...
static void aw881xx_parse_rename_dt(struct aw881xx *aw881xx,
					struct device_node *np)
{
	int ret = -EINVAL;
	uint32_t rename_value;

	ret = of_property_read_u32(np, "rename-flag", &rename_value);
	if (ret < 0) {
		aw_dev_info(aw881xx->dev,
			"%s:read rename_flag failed, use default: 0\n",
			__func__);
		rename_value = AW881XX_RENAME_VAL_DEFAULT;
	} else {
		aw_dev_info(aw881xx->dev, "%s: read rename_flag is : %d\n",
			__func__, rename_value);
	}

	aw881xx->rename_flag = rename_value;
}
				...
				struct device_node *np)
{
	...
	aw881xx_parse_rename_dt(aw881xx,np);
				...
				size_t count)
{
	...
	if (buf == NULL) {
		aw_dev_err(aw881xx->dev, "buf is NULL");
		return -EINVAL;
	}
 ...
 *
 ******************************************************/
static int aw881xx_codec_register(struct aw881xx *aw881xx)
{
	int ret = -EINVAL;
	struct snd_soc_dai_driver *dai = NULL;

	dai = devm_kzalloc(aw881xx->dev, sizeof(aw881xx_dai), GFP_KERNEL);
	if (!dai)
		return -ENOMEM;
	memcpy(dai, aw881xx_dai, sizeof(aw881xx_dai));

	if(aw881xx->rename_flag == AW_DEV_RENAME_ENABLE) {
		ret = aw881xx_append_channel_suffix("%s-%d", &dai->name, aw881xx);
		if (ret < 0)
			return ret;
		ret = aw881xx_append_channel_suffix("%s_%d",
		...
		if (ret < 0)
			return ret;
		dev_set_name(aw881xx->dev, "%s_%d",
			"aw881xx_smartpa", aw881xx->channel);
		aw_dev_info(aw881xx->dev, "change dev_name:%s",
			dev_name(aw881xx->dev));

	}else {
		ret = aw881xx_append_i2c_suffix("%s-%d-%x", &dai->name, aw881xx);
		if (ret < 0)
			return ret;
		ret = aw881xx_append_i2c_suffix("%s_%d_%x",
			&dai->playback.stream_name, aw881xx);
		if (ret < 0)
			return ret;
		ret = aw881xx_append_i2c_suffix("%s_%d_%x",
	...
	ret = aw_componet_codec_ops.aw_snd_soc_register_codec(aw881xx->dev,
					&soc_codec_dev_aw881xx,
					dai, ARRAY_SIZE(aw881xx_dai));
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s failed to register aw881xx: %d\n",
				__func__, ret);
		return ret;
	}

	if(aw881xx->rename_flag == AW_DEV_RENAME_ENABLE) {
		dev_set_name(aw881xx->dev, "%d-00%x",
			aw881xx->i2c->adapter->nr, aw881xx->i2c->addr);
		aw_dev_info(aw881xx->dev, "reset dev_name:%s",
			dev_name(aw881xx->dev));
	}
				...
				const struct i2c_device_id *id)
{
	...
	/* register codec */
	ret = aw881xx_codec_register(aw881xx);
	if (ret < 0)
		return ret;
	...
	aw881xx->volume_desc.ctl_volume = AW881XX_VOLUME_MAX;
	aw881xx->volume_desc.monitor_volume = AW881XX_VOLUME_MAX;
