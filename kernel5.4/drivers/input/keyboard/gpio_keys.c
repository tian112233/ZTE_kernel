...
static void cali_powerkey_timer_func(struct timer_list *t)
{
	schedule_work(&powerkey_reboot_work);
}
/*zte add: reboot after powerkey pressed for 3s in cali mode end*/
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (button->code == 116) {
		if (state == 1) {
			pr_info("%s:zte powerkey pressed!\n", __func__);
			if (zte_cali_mode == true)
				mod_timer(&cali_powerkey_timer, jiffies + 3 * HZ);
			else {
				#ifdef CONFIG_ZTE_SW_LONG_PRESS_REBOOT
				mod_timer(&cali_powerkey_timer, jiffies + 9 * HZ);
				#endif
			}
		} else if (state == 0) {
			pr_info("%s:zte powerkey released!\n", __func__);
			if (zte_cali_mode == true)
				del_timer(&cali_powerkey_timer);
			else {
				#ifdef CONFIG_ZTE_SW_LONG_PRESS_REBOOT
				del_timer(&cali_powerkey_timer);
				#endif
	...
	if (button->code == 114) {
		if (state == 1) {
			pr_info("%s:zte vol down pressed!\n", __func__);
		} else if (state == 0) {
			pr_info("%s:zte vol down released!\n", __func__);
		}
	}
	if (button->code == 115) {
		if (state == 1) {
			pr_info("%s:zte vol up pressed!\n", __func__);
		} else if (state == 0) {
			pr_info("%s:zte vol up released!\n", __func__);
		}
	}
	/*zte add: reboot after powerkey pressed for 3s in cali mode end*/
				...
				struct fwnode_handle *child)
{
	...
	if (bdata->gpiod) {
		...
		/*zte add: reboot after powerkey pressed for 3s in cali mode start*/
		INIT_WORK(&powerkey_reboot_work, powerkey_reboot_work_func);
		timer_setup(&cali_powerkey_timer,
			    cali_powerkey_timer_func, 0);
		/*zte add: reboot after powerkey pressed for 3s in cali mode end*/
