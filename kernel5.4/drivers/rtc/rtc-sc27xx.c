...
static int sprd_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	pr_info("%s begin enabled=%d, time=%ld, secs=%ld", __func__, alrm->enabled, alrm->time, secs);
	...
	if (!rtc->rtc->aie_timer.enabled || rtc_tm_sub(&aie_time, &alrm->time)) {
		pr_info("%s rtc->rtc->aie_timer.enabled=%d", __func__, rtc->rtc->aie_timer.enabled);
...
static int sprd_rtc_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	...
	if (enabled) {
		...
		pr_info("%s sprd_rtc_set_secs ret=%d", __func__, ret);
