...
static int st21nfc_clock_select(struct st21nfc_device *st21nfc_dev)
{
	...
	nclk = devm_clk_get(&st21nfc_dev->client->dev, "clk_aux1");
	pclk = devm_clk_get(&st21nfc_dev->client->dev, "ext_26m");
	st21nfc_dev->s_clk = devm_clk_get(&st21nfc_dev->client->dev, "aux1_eb");
	// if NULL we assume external crystal and dont fail
	if (IS_ERR_OR_NULL(st21nfc_dev->s_clk))
		return 0;
	clk_set_parent(nclk, pclk);
	clk_set_rate(nclk, 26000000);
	if (st21nfc_dev->clk_run == false) {
		clk_prepare_enable(nclk);
...
static int zte_st21nfc_hw_check(struct i2c_client *client, struct st21nfc_device *st21nfc_dev)
{
	int ret = 0;
	int j = 0;
	bool readOk = false;
	int is4bytesheader = 1;
	uint8_t buffer[260];
	bool rst_hw = false;
	...
	do {
		...
		int bytesRead = i2c_master_recv(client, buffer, hdrsz);

		readOk = false;
		if (bytesRead == hdrsz) {
			// if(bytesRead == 3) pr_info("%s receive header:%02x%02x%02x\n\n",
			//  __func__, buffer[0],buffer[1],buffer[2]);
			// else if (bytesRead == 4) pr_info("%s receive header:%02x%02x%02x%02x\n\n",
			...
			if ((buffer[0] != 0x7E) && (buffer[1] != 0x7E)) {
				...
				if (buffer[1] != 0x7E) {
					buffer[0] = buffer[1];
					buffer[1] = buffer[2];
					bytesRead = i2c_master_recv(client, buffer + 2, 1);
					if (bytesRead == 1) {
						readOk = true;
					} else {
						pr_err("%s Failed to read last byte\n", __func__);
					}
				} else if (buffer[2] != 0x7E) {
					buffer[0] = buffer[2];
					bytesRead = i2c_master_recv(client, buffer + 1, 2);
					if (bytesRead == 2) {
						readOk = true;
					} else {
						pr_err("%s Failed to read last 2 bytes\n", __func__);
					}
				} else {
					pr_warn("%s received idle data\n", __func__);
				}
			}
			...
			if (readOk == true) {
				...
				if (remaining - extra > 0) {
					bytesRead =
						i2c_master_recv(client, buffer + 3 + extra, remaining - extra);
				}
				if (bytesRead == remaining - extra) {
					char tmpStr[521] = { 0x00 };
					int i = 0;

					for (i = 0; i < bytesRead + 3 + extra; i++) {
						snprintf(&tmpStr[i*2], 3, "%02X", buffer[i]);
					}
					...
					msleep(20);
					pr_err(
						"%s ! didn't read expected bytes from "
						"i2c,bytesRead=%d,remaining=%d,extra=%d,is4bytesheader=%d\n", __func__,
						bytesRead, remaining, extra, is4bytesheader);
				}
			} else {
				msleep(20);
				pr_err("%s !readOk; bytesRead=%d, buffer: 0x%02x 0x%02x 0x%02x\n", __func__,
						bytesRead, buffer[0], buffer[1], buffer[2]);
			}

		} else {
			msleep(20);
			pr_err("%s ! didn't read %d requested bytes from i2c\n", __func__, hdrsz);
		}

		memset(buffer, 0xca, sizeof(buffer));

		j++;
	} while ((j < 20) && gpiod_get_value(st21nfc_dev->gpiod_irq));
...
/* QCOM and MTK54 use standard GPIO definition */
	...
	pr_debug(
		"%s : debug irq_gpio = %d, client-irq =  %d, reset_gpio = %d, clkreq_gpio = %d\n",
		__func__,
		IS_ERR_OR_NULL(st21nfc_dev->gpiod_irq) ?
			-1 : desc_to_gpio(st21nfc_dev->gpiod_irq),
		client->irq,
		IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset) ?
			-1 : desc_to_gpio(st21nfc_dev->gpiod_reset),
		IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq) ?
			-1 : desc_to_gpio(st21nfc_dev->gpiod_clkreq));
	ret = zte_st21nfc_hw_check(client, st21nfc_dev);
