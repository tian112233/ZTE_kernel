...
static int zte_st54spi_hw_check(struct spi_device *spi, struct st54spi_data *st54spi)
{
	dev_info(&spi->dev, "%s : Remove zte_st54spi_hw_check\n", __func__);
	return 0;
}
...
static int st54spi_probe(struct spi_device *spi)
{
	...
	status = zte_st54spi_hw_check(spi, st54spi);
	dev_info(&spi->dev, "%s : hardware check status %d\n", __func__, status);
	if (status) {
		st54spi->spi = NULL;
		st54spi->spi_reset = NULL;
