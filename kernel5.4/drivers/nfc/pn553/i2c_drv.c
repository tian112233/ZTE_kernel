...
static const char *nfc_clkirq_gpio_name = "nfc_clkirq_gpio";
...
int i2c_disable_irq(struct nfc_dev *dev)
{
	unsigned long flags;
	//zte add
	if (gpio_get_value(dev->configs.gpio.clkreq_gpio)) {
...
int i2c_read(struct nfc_dev *nfc_dev, char *buf, size_t count, int timeout)
{
	...
	if (!gpio_get_value(nfc_gpio->irq)) {
		while (1) {
			...
			//zte add
			if (gpio_get_value(nfc_gpio->clkreq_gpio)) {
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(nfc_gpio->clkreq_gpio)) {
		ret = gpio_request(nfc_gpio->clkreq_gpio, nfc_clkirq_gpio_name);
		if (ret) {
			pr_err("%s: unable to request nfc clkreq_gpio [%d]\n",
						__func__, nfc_gpio->clkreq_gpio);
			goto err_clkirq_gpio;
		}
		ret = gpio_direction_input(nfc_gpio->clkreq_gpio);
		if (ret) {
			pr_err("%s: unable to set direction for nfc irq gpio [%d]\n",
						__func__, nfc_gpio->clkreq_gpio);
			goto err_clkirq_gpio;
		}
		clkirqn = gpio_to_irq(nfc_gpio->clkreq_gpio);
		if (clkirqn < 0) {
...
err_request_irq_failed:
	nfc_misc_unregister(nfc_dev, DEV_COUNT);
err_clkirq_gpio:
	gpio_free(nfc_gpio->clkreq_gpio);
err_free_nfc_dev:
	kfree(nfc_dev);
err:
