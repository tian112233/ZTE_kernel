...
extern struct clk *bb_clk;
extern struct clk *source;
extern struct clk *enable;
#endif

static const char *nfc_gpio_name = "nfc_gpio";
		 ...
		 uint8_t interface)
{
	...
	nfc_gpio->clkreq_gpio = of_get_named_gpio(np, "nxp,pn544-clkreq", 0);
	if ((!gpio_is_valid(nfc_gpio->clkreq_gpio)))
			nfc_gpio->clkreq_gpio =  -EINVAL;
	pr_info("%s: %d, %d, %d, %d\n", __func__, nfc_gpio->irq, nfc_gpio->ven,
		nfc_gpio->dwl_req, nfc_gpio->clkreq_gpio);
	return 0;
}
...
int configure_gpio(unsigned int gpio, int flag)
{
	...
	if (gpio_is_valid(gpio)) {
		ret = gpio_request(gpio, nfc_gpio_name);
		if (ret) {
...
static int nfc_ioctl_power_states(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	#ifdef CONFIG_NFC_ENABLE_BB_CLK
		if (bb_clk == NULL) {
                           pr_err("%s bb_clk = NULL! \n", __func__);
                        }
                if (enable == NULL) {
                           pr_err("%s enable = NULL! \n", __func__);
                        }
                clk_disable_unprepare(bb_clk);
                clk_disable_unprepare(enable);
                pr_info("%s: aux1 clk off\n", __func__);
	#endif
