...
#ifdef CONFIG_VENDOR_ZTE_UNDER_PANEL_LIGHT
...
static int als_cali_coef = CALIB_STATUS_FAIL;  /* add for light factory cali data collected */
static int ps_cali_ct = 0;  /* add for prox factory cali data collected */
...
static void shub_cali_callback(struct shub_data *sensor, u8 *data, u32 len)
{
	int cali_data[3] = {0};
	int sensor_type = sensor->cal_id;
	...
	case SENSOR_PROXIMITY:
		zlog_client_record(zlog_sprd_sensorhub_client, "prox cali data: %d %d\n", cali_data[0], cali_data[1]);
	break;
	}
	zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_FACTORY_CALI_NO);
	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d cali_data = %d %d send to zlog\n",
			__func__, sensor_type, cali_data[0], cali_data[1]);
#endif

	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d sensor->cali_data = [%d %d]\n",
			__func__, sensor_type, cali_data[0], cali_data[1]);
	shub_send_event_to_iio(sensor, (u8 *)&sensor->cali_store, sizeof(sensor->cali_store));
}
					...
					int sensor_type, u8 *data)
{
	...
	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d sensor->cali_data = [%d %d %d]\n",
			__func__, sensor_type, cali_data[0], cali_data[1], cali_data[2]);
	err = shub_send_event_to_iio(sensor, (u8 *)&sensor->cali_store, sizeof(sensor->cali_store));

/* add for light factory cali data collected start */
	if (sensor_type == SENSOR_LIGHT) {
		als_cali_coef = 10000000 / cali_data[0];
		dev_info(&sensor->sensor_pdev->dev, "%s: als_cali_coef set to %d\n", __func__, als_cali_coef);
	}
/* add for light factory cali data collected end */

/* add for prox factory cali data collected start */
	if (sensor_type == SENSOR_PROXIMITY) {
		ps_cali_ct = cali_data[0];
		dev_info(&sensor->sensor_pdev->dev, "%s: ps_cali_ct set to %d\n", __func__, ps_cali_ct);
	}
/* add for prox factory cali data collected end */
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (sensor->cal_cmd == CALIB_DATA_READ) {
			...
			if (err < 0) {
				dev_err(&sensor->sensor_pdev->dev,
					"Save Sensor CalibratorData Fail\n");
				return err;
			}
			return sprintf(buf, "%d\n", err);
	}
				     ...
				     const char *buf, size_t count)
{
	...
	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d cali_data = %d %d %d send to sensorhub\n",
			__func__, sensor_type, cali_data[0], cali_data[1], cali_data[2]);

/* add for light factory cali data collected start */
	if (sensor_type == SENSOR_LIGHT) {
		als_cali_coef = 10000000 / cali_data[0];
		dev_info(&sensor->sensor_pdev->dev, "%s: als_cali_coef init to %d\n", __func__, als_cali_coef);
	}
/* add for light factory cali data collected end */
...
/* add for prox factory cali data collected start */
	if (sensor_type == SENSOR_PROXIMITY) {
		ps_cali_ct = cali_data[0];
		dev_info(&sensor->sensor_pdev->dev, "%s: ps_cali_ct init to %d\n", __func__, ps_cali_ct);
	}
/* add for prox factory cali data collected end */
...
/* add for light factory cali data collected start */
static ssize_t als_cali_coef_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 10, "%d\n", als_cali_coef);
}
static DEVICE_ATTR_RO(als_cali_coef);
/* add for light factory cali data collected end */

/* add for prox factory cali data collected start */
static ssize_t ps_cali_ct_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 10, "%d\n", ps_cali_ct);
}
static DEVICE_ATTR_RO(ps_cali_ct);
/* add for prox factory cali data collected end */
...
static struct attribute *sensorhub_attrs[] = {
	...
	&dev_attr_als_cali_coef.attr,  /* add for light factory cali data collected */
	&dev_attr_ps_cali_ct.attr,  /* add for prox factory cali data collected */
...
static int shub_algo_cmd_notifier_fn(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	/* add for sensorhub init ready */
	wait_event_interruptible(sensor->sensorhub_wq,
						sensor->sensorhub_init_ready);
...
static int shub_probe(struct platform_device *pdev)
{
	...
	mcu->cali_callback = shub_cali_callback;  /* add for prox cali base save */
