...
int ptn38003a_dp_mode_set(unsigned int value)
{
	...
	/* dp operating mode set */
	if (value) {
		mask = PTN38003A_OPERATION_MODE_MASK;
		ret = ptn38003a_reg_update_bits(PTN38003A_MODE_CONTROL, mask, 0x02);
	} else {
	/*
	 *in the situation of docking station, if dp set the redriver mode value 0,
	 *it will make usb3.1/3.0 not work, so dp don`t set the redriver mode value 0 now
	 */
		mask = PTN38003A_OPERATION_MODE_MASK;
		ret = ptn38003a_reg_update_bits(PTN38003A_MODE_CONTROL, mask, 0x00);
	}

	if (ret < 0) {
		dev_err(ptn38003a_redriver->dev,
					"set PTN38003A_CONTROL_MODE failed: %d.\n", ret);
		goto err;
	}
...
EXPORT_SYMBOL(ptn38003a_dp_mode_set);
...
int ptn38003a_dp_lane_set(unsigned int value)
{
	...
	ptn38003a_read(PTN38003A_DP_LINK_CONTROL_AND_STATUS, &val);
	printk("ptn38003a_dp_lane_count_start:0x%x,0x%x\n", val, value);

	/* dp operating lane set */
	mask = PTN38003A_DP_LANE_COUNT_MASK;
	if (value == 0x81 || value == 0x01)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x04);
	else if (value == 0x82 || value == 0x02)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x08);
	else if (value == 0x84 || value == 0x04)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x0c);
	else if (value == 0)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x00);

	ptn38003a_read(PTN38003A_DP_LINK_CONTROL_AND_STATUS, &val);
	printk("ptn38003a_dp_lane_count_end:0x%x\n", val);

	if (ret < 0) {
		dev_err(ptn38003a_redriver->dev,
					"set PTN38003A_DP_LANE failed: %d.\n", ret);
		goto err;
	}
...
EXPORT_SYMBOL(ptn38003a_dp_lane_set);
...
int ptn38003a_dp_link_rate_set(unsigned int value)
{
	...
	ptn38003a_read(PTN38003A_DP_LINK_CONTROL_AND_STATUS, &val);
	printk("ptn38003a_dp_link_rate_start:0x%x,0x%x\n", val, value);

	/* dp link rate set */
	mask = PTN38003A_DP_LINK_RATE_MASK;
	if (value == 0x06)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x00);
	else if (value == 0x0a)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x01);
	else if (value == 0x14)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x02);
	else if (value == 0x1e)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x03);
	else if (value == 0)
		ret = ptn38003a_reg_update_bits(PTN38003A_DP_LINK_CONTROL_AND_STATUS, mask, 0x00);

	ptn38003a_read(PTN38003A_DP_LINK_CONTROL_AND_STATUS, &val);
	printk("ptn38003a_dp_link_rate_end:0x%x\n", val);

	if (ret < 0) {
		dev_err(ptn38003a_redriver->dev,
					"set PTN38003A_DP_LINK_RATE failed: %d.\n", ret);
		goto err;
	}
...
EXPORT_SYMBOL(ptn38003a_dp_link_rate_set);
