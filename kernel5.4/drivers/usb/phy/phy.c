...
static void usb_phy_notify_charger_work(struct work_struct *work)
{
	...
	pr_info("sprd: %s[%d], usb_phy->chg_state=%d\n", __func__,
		__LINE__, usb_phy->chg_state);
	switch (usb_phy->chg_state) {
	case USB_CHARGER_PRESENT:
		usb_phy_get_charger_current(usb_phy, &min, &max);

		pr_info("sprd: %s[%d], max=%d\n", __func__, __LINE__, max);
...
{
	dev_info(usb_phy->dev, "sprd: %s[%d], state=%d, usb_phy->chg_state=%d\n",
		__func__, __LINE__, state, usb_phy->chg_state);
