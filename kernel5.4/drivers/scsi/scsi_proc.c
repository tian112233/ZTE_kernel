...
extern u64 ufshcd_get_device_capacity(struct scsi_device *sdev);
...
static int proc_print_emmc_id(struct device *dev, void *data)
{
	...
	//if (sdev->host->hostt->device_capacity) {
	    capacity_512 = ufshcd_get_device_capacity(sdev);
	//}
...
typedef struct _ddr_type_info {
	int id;
	char *type;
} ddr_type_info;

ddr_type_info ddr_type_list[] = {
	{0x01, "DDR1"},
	{0x02, "LPDDR2"},
	{0x03, "LPDDR3"},
	{0x04, "PCDDR3"},
...
static int vendor_proc_print_ddr_id(struct device *dev, void *data)
{
	struct scsi_device *sdev;
	struct seq_file *s = data;
	...
	ptr = strstr(saved_command_line, "zte_dram_type=");
	if (ptr) {
	ptr += strlen("zte_dram_type=");
	ddr_id = simple_strtol(ptr, NULL, 16);
	for (i = 0; i < ARRAY_SIZE(ddr_type_list); i++) {
		if (ddr_type_list[i].id == ddr_id) {
			ddr_type = ddr_type_list[i].type;
			break;
		}
	}
	}
...
//printk(KERN_ERR "%s: manfname is  %s, ddr_type is  %s\n", __func__, manfname,ddr_type);
	if (sdev->lun == 0) {
...
static int proc_ddr_id_show(struct seq_file *sfile, void *dev)
{
	return vendor_proc_print_ddr_id(dev, sfile);
}

static const struct seq_operations proc_ddr_id_ops = {
	.start	= scsi_seq_start,
	.next	= scsi_seq_next,
	.stop	= scsi_seq_stop,
	.show	= proc_ddr_id_show
};

static int proc_ddr_id_open(struct inode *inode, struct file *file)
{
	/*
	 * We don't really need this for the write case but it doesn't
	 * harm either.
	 */
	return seq_open(file, &proc_ddr_id_ops);
}

static const struct file_operations proc_ddr_id_operations = {
	.owner		= THIS_MODULE,
	.open		= proc_ddr_id_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release,
};
...
void init_ufs_id_proc(void)
{
	struct proc_dir_entry *pde;

	pde = proc_create("driver/emmc_id", 0444, NULL, &proc_emmc_id_operations);
	if (!pde) {
		goto err2;
	}
	pde = proc_create("driver/ddr_id", 0444, NULL, &proc_ddr_id_operations);
	return;
